...
/**
 * RawPHP_Sniffs_ControlStructures_SwitchDeclarationSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class RawPHP_Sniffs_ControlStructures_SwitchDeclarationSniff
    implements PHP_CodeSniffer_Sniff
{
    ...
    public function process( PHP_CodeSniffer_File $phpcsFile, $stackPtr )
    {
        ...
        while ( ($nextCase = $phpcsFile->findNext( array( T_CASE, T_DEFAULT, T_SWITCH ),
        ($nextCase + 1 ), $switch[ 'scope_closer' ] )) !== FALSE )
        {
            ...
            if ( $tokens[ $nextCase ][ 'content' ] !==
                strtolower( $tokens[ $nextCase ][ 'content' ] )
            )
            {
                $expected = strtolower( $tokens[ $nextCase ][ 'content' ] );
                $error = strtoupper( $type ) .
                    ' keyword must be lowercase; expected "%s" but found "%s"';
                $data = array(
                    $expected,
                    $tokens[ $nextCase ][ 'content' ],
                );
                $phpcsFile->addError( $error, $nextCase, $type . 'NotLower',
                    $data );
            }
            ...
            {
                $error = strtoupper( $type ) .
                    ' keyword must be indented 4 spaces from SWITCH keyword';
            ...
            {
                $error = 'There must be no space before the colon in a ' .
                    strtoupper( $type ) . ' statement';
                $phpcsFile->addError( $error, $nextCase,
                    'SpaceBeforeColon' . $type );
            }
            ...
            )
            {
                if ( $tokens[ $nextBreak ][ 'scope_condition' ] === $nextCase )
                {
                    ...
                    for ( $i = ($semicolon + 1);
                            $i < $tokens[ $stackPtr ][ 'scope_closer' ]; $i++ )
                    {
                        if ( $tokens[ $i ][ 'type' ] !== 'T_WHITESPACE' )
                        {
                    ...
                    if ( $type === 'Case' )
                    {
                        ...
                        if ( $nextLine !== ($breakLine + 2) && $i !== 
                            $tokens[ $stackPtr ][ 'scope_closer' ]
                            )
                        {
                            $error = 'Case breaking statements must be ';
                            $error .= 'followed by a single blank line';
                    ...
                    else
                    {
                        ...
                        {
                            $error = 'Blank lines are not allowed after the ';
                            $error .= 'DEFAULT case\'s breaking statement';
                            $phpcsFile->addError( $error, $nextBreak,
                                'SpacingAfterDefaultBreak' );
                        }
                    }
