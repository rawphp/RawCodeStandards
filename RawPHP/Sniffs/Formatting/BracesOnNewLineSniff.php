...
/**
 * PHP_CodeSniffer_RawPHP_Formatting_BracesOnNewLineSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class RawPHP_Sniffs_Formatting_BracesOnNewLineSniff
    implements PHP_CodeSniffer_Sniff
{
    ...
    public function process( PHP_CodeSniffer_File $phpcsFile, $stackPtr )
    {
        ...
        if ( in_array( $tokens[ $stackPtr ][ 'code' ], $registered ) )
        {
            ...
            //    $phpcsFile->addError( $error, $stackPtr,
            //        'OpeningBraceWithSameIndentation' );
            //}
            ...
            //    $phpcsFile->addError( $error, $stackPtr,
            //        'ClosingBraceWithSameIndentation' );
            //}
            // check eol right after
            if ( isset( $tokens[ $closer + 1 ] ) && $tokens[ $closer + 1 ][ 'content' ]
                !== $phpcsFile->eolChar )
            {
                $error = 'Closing brace must be followed by EOL char';
                $phpcsFile->addError( $error, $stackPtr,
                    'ClosingBraceFollowedByEOL' );
            }
        }
    }
